#!/usr/bin/env python
"""Prepare and optionally run a FABLE FreshForge rebuild workflow."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    strategy = "all-columns" if args.all_columns else args.output_ref_strategy
    ...
    if strategy == "all-columns":
        ...
        column_flavour_tags = requested_tags[0] if len(requested_tags) == 1 else tuple(requested_tags)
    ...
    if args.run:
        ...
        if not run_payload["ok"]:
            return _emit(_summary(plan, repo_root, run_payload=run_payload, strategy=strategy), args.json_output, 1)

    return _emit(_summary(plan, repo_root, run_payload=run_payload, strategy=strategy), args.json_output)


def _prepare_rebuild(**kwargs: Any) -> Any:
    from fable_pyculator import prepare_freshforge_rebuild

    return prepare_freshforge_rebuild(**kwargs)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare FABLE output refs, cached-workbook validation scenario, and Modelwright "
            "FreshForge workflow. Use --run to execute the FreshForge workflow."
        )
    )
    parser.add_argument("--repo-root", type=Path, default=None, help="Repository root. Defaults to auto-detection.")
    parser.add_argument("--workbook-version", default="2021", help="FABLE workbook version, such as 2020 or 2021.")
    parser.add_argument(
        "--workbook-path",
        default=None,
    ...
    parser.add_argument(
        "--artifact-dir",
        default=None,
        help="Ignored artifact directory. Defaults to tmp/generated-models/fable-{version}.",
    )
    parser.add_argument(
        "--workflow-filename",
    ...
    parser.add_argument(
        ...
        help="Named output-ref strategy. Defaults to output-columns.",
    )
    parser.add_argument(
        "--column-flavour-tag",
        action="append",
    ...
    parser.add_argument(
        "--all-columns",
        action="store_true",
        help="Alias for --output-ref-strategy all-columns.",
    )
    parser.add_argument(
        "--table-name",
        action="append",
    ...
    parser.add_argument("--module-name", default=None, help="Generated module name. Defaults by workbook version.")
    parser.add_argument("--workflow-id", default=None, help="FreshForge workflow id. Defaults by workbook version.")
    parser.add_argument("--workflow-name", default=None, help="FreshForge workflow name. Defaults by workbook version.")
    parser.add_argument(
        "--workflow-description",
        default=None,
        help="FreshForge workflow description. Defaults by workbook version.",
    )
    parser.add_argument("--scenario-id", default=None, help="Validation scenario id. Defaults by workbook version.")
    parser.add_argument(
...
def _summary(plan: Any, repo_root: Path, *, run_payload: dict[str, Any] | None, strategy: str) -> dict[str, Any]:
    paths = plan.paths
...
def _emit(payload: dict[str, Any], json_output: bool, exit_code: int = 0) -> int:
    if json_output:
        ...
        print("FABLE FreshForge rebuild")
        print(f"Mode: {payload['mode']}")
        print(f"Output-ref strategy: {payload['output_ref_strategy']}")
